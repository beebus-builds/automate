...
export function safeUrl(url: unknown, opts: { allowDataImage?: boolean } = {}): string {
  const s = String(url || '').trim();
  if (!s || s === '#') return '#';
  if (s.startsWith('/') || s.startsWith('#')) return escHtml(s);
  if (/^https?:\/\//i.test(s)) return escHtml(s);
  if (s.startsWith('//')) return escHtml(s);
  if (opts.allowDataImage && /^data:image\/(png|jpeg|jpg|gif|webp);base64,/i.test(s)) return escHtml(s);
  return '#';
}

function sanitizeHtmlBlock(html: string): string {
  if (!html) return '';
  let out = String(html).replace(/<script[\s\S]*?<\/script\s*>/gi, '');
  ...
  // strip data:image/svg+xml
  out = out.replace(/src\s*=\s*["']\s*data:image\/svg\+xml[^"']*["']/gi, 'src="#"');
  return out;
}

export function sanitizeCustomHead(head: unknown): string {
  const s = String(head || '').trim();
  if (!s) return '';
  // only allow <meta> and <link> tags, strip everything else
  const allowed = s.match(/<(meta|link)\b[^>]*>/gi);
  if (!allowed) return '';
  // sanitize each tag: remove event handlers and javascript: src
  return allowed
...
function renderBlock(b: SectionBlock, cls: string): string {
  ...
  switch (b.type) {
    ...
    case 'image': {
      const safeSrc = safeUrl(b.src || '', { allowDataImage: true });
      return `<div class="${cls}__imgwrap"><img src="${safeSrc}" alt="${escHtml(b.alt || '')}" style="width:100%;border-radius:${rad};object-fit:cover;display:block;background:#111827;min-height:120px" loading="lazy" /></div>`;
    }
    case 'button': {
      const safeHref = safeUrl(b.href || '#');
      return `<a class="${cls}__btn" href="${safeHref}" target="${safeHref.startsWith('http') ? '_blank' : '_self'}" rel="noopener" style="display:inline-block;padding:.7rem 1.5rem;border-radius:999px;font-weight:700;font-size:.8rem;color:#fff;text-decoration:none;background:linear-gradient(135deg,${PRIMARY},${ACCENT});box-shadow:0 4px 16px rgba(0,0,0,0.25);transition:transform .15s">${escHtml(b.label || '')}</a>`;
    }
    case 'card':
    ...
    case 'html': {
      // Teacher-authored markup — sanitized: strips scripts, event handlers, javascript: URLs
      return `<div class="${cls}__html">${sanitizeHtmlBlock(b.html || '')}</div>`;
    }
    case 'booking': {
    ...
    case 'file': {
      const href = safeUrl(b.src || '#');
      return `<a href="${href}" ${/^https?:\/\//i.test(String(b.src||'')) ? 'target="_blank" rel="noopener"' : ''} download style="display:flex;align-items:center;gap:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:${rad};padding:16px 18px;text-decoration:none;transition:transform .15s">
    ...
    case 'video': {
      ...
      const inner = yt
        ...
        : vm
          ...
          : url
            ? `<video src="${safeUrl(url, { allowDataImage: false })}" controls preload="metadata" style="width:100%;border-radius:${rad};display:block;background:#000;max-height:420px"></video>`
...
export function scopeCss(css: string, cls: string): string {
  // Strip comments + script/closing-style breakouts + @import (exfil) for safety.
  const clean = css
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/<\/style\s*>/gi, '')
